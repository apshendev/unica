...
Когда что: существенная переработка запроса (добавить поля, соединения, переписать пакет) -> получи запрос через `unica.view` on the schema node и возьми `data.dataSets[].query`; отредактируй текст и передай его в `set-query` как `Value`. `query` отдаёт сырой текст запроса целиком, с отступами строк продолжения `|`, поэтому передача точна, включая многопакетные запросы. Точечная замена идентификатора или подстроки не требует выгрузки: используй `patch-query`.
