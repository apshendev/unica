...
Справочник типовых паттернов дизайна управляемых форм 1С. Используй **перед** созданием формы через `unica.apply` с операциями `form.create` и `element.add`, когда требования пользователя не детализируют расположение элементов.
